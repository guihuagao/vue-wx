<template>
  <div class="update-notice">
    ...
    <div class="up-notice-top">
      ...
      &nbsp;{{notice.noticeTitle}}<span class="notice-delete" @click="deleteNoticeItem(notice)">删除</span>
    </div>
    <div class="up-notice-center">
      <div class="notice-text">{{titleLeft}}标题</div>
      <div class="notice-default-div notice-title">
        <div>{{titleLeft}}标题</div>
    ...
    <div class="notice-bottom">
      <button class="button-default" @click="updateNotice()"><label>确认修改</label></button>
    </div>
    <mt-datetime-picker
      ref="picker"
      type="datetime"
  ...
  export default {
    ...
    methods: {
      ...
      queryTnfo(){
        ...
        if(this.notice.noticeType==0){
          this.headerName='修改公告';
          this.titleLeft='公告';
        }else if(this.notice.noticeType==1){
          this.headerName='修改通知';
          this.titleLeft='通知';
        }else if(this.notice.noticeType==2){
          this.headerName='修改群规';
          this.titleLeft='群规';
        }

        this.title=this.notice.noticeTitle;
        this.content=this.notice.noticeContent;

        if(this.notice.isTop==1){
           this.isTop=true;
        }else{
          this.isTop=false;
        }

        if(this.notice.isAtAll==1){
          this.isAtAll=true;
        }else{
          this.isAtAll=false;
        }

        if(this.notice.isRead==1){
          this.isUrl=true;
        }else{
          this.isUrl=false;
        }
      ...
      /*删除*/
      deleteNoticeItem(notice){
        var vm = this;
        this.$http.get('/xiaov/notice/delNotice.do',{params:{id:notice.id}}).then(function(res) {
          if(res.data.success){
      ...
      updateNotice(){
        ...
        * type
          ...
          isAtAll
        * */
        this.$http.get('/xiaov/notice/updateNotice.do',{params:{id:vm.notice.id,type:vm.selectId,title:vm.title,content:vm.content,publishTime:vm.publishTime,isTop:vm.Top,isUrl:vm.Url,isAtAll:vm.AtAll}}).then(function(res) {
          if(res.data.success){
            Toast(res.data.message);
